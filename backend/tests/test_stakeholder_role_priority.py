"""
Tests for stakeholder role priority: CTO, CIO, CFO, CMO are primary targets.
TDD: Tests written FIRST before implementation.

Rules:
1. CTO, CIO, CFO, CMO are shown as primary executives (full profile cards)
2. All other roles (CISO, CPO, CEO, COO, VP, Director, Manager) are otherContacts
3. If NONE of the primary 4 are found, the best available roles become primary
4. Role sorting order: CTO > CIO > CFO > CMO > others
"""
import pytest
import sys
import os
...
class TestPriorityRoles:
    ...
    def test_primary_roles_are_cto_cio_cfo_cmo(self):
        """PRIMARY_STAKEHOLDER_ROLES contains exactly CTO, CIO, CFO, CMO."""
        from production_main import PRIMARY_STAKEHOLDER_ROLES
        assert PRIMARY_STAKEHOLDER_ROLES == {"CTO", "CIO", "CFO", "CMO"}
    ...
    def test_cmo_is_primary_role(self):
        """CMO is classified as a primary stakeholder role."""
        from production_main import PRIMARY_STAKEHOLDER_ROLES
        assert "CMO" in PRIMARY_STAKEHOLDER_ROLES
    ...
    def test_coo_is_NOT_primary_role(self):
        """COO is NOT a primary role (falls to otherContacts)."""
        from production_main import PRIMARY_STAKEHOLDER_ROLES
        assert "COO" not in PRIMARY_STAKEHOLDER_ROLES
...
class TestStakeholderMapSplit:
    ...
    def test_ceo_goes_to_other_contacts_when_primaries_present(self):
        """CEO goes to otherContacts when CTO/CIO/CFO/CMO are present."""
    ...
    def test_fallback_when_no_primary_roles_found(self):
        """When no CTO/CIO/CFO/CMO exist, other roles become primary."""
        data = self._build_stakeholders_data(["CISO", "CEO", "COO"])
        result = self._split_stakeholders(data)
    ...
    def test_fallback_promotes_only_first_four(self):
        ...
        data = self._build_stakeholders_data(["CISO", "CEO", "COO", "VP", "Director"])
        result = self._split_stakeholders(data)
    ...
    def test_all_four_primary_roles_found(self):
        ...
        data = self._build_stakeholders_data(["CTO", "CIO", "CMO", "CFO", "CISO"])
        result = self._split_stakeholders(data)
        primary_roles = [s["role_type"] for s in result["stakeholders"]]
        assert set(primary_roles) == {"CTO", "CIO", "CMO", "CFO"}
...
class TestRolePriorityOrder:
    ...
    def test_cfo_third_priority(self):
        ...
        from production_main import ROLE_PRIORITY
        assert ROLE_PRIORITY.get("cfo", 99) < ROLE_PRIORITY.get("cmo", 99)
        assert ROLE_PRIORITY.get("cfo", 99) < ROLE_PRIORITY.get("ciso", 99)

    def test_cmo_fourth_priority(self):
        """CMO is fourth, before all secondary roles."""
        from production_main import ROLE_PRIORITY
        assert ROLE_PRIORITY.get("cmo", 99) < ROLE_PRIORITY.get("ciso", 99)
        assert ROLE_PRIORITY.get("cmo", 99) < ROLE_PRIORITY.get("ceo", 99)
        assert ROLE_PRIORITY.get("cmo", 99) < ROLE_PRIORITY.get("coo", 99)
        assert ROLE_PRIORITY.get("cmo", 99) < ROLE_PRIORITY.get("cpo", 99)

    def test_sort_produces_correct_order(self):
        """Sorting stakeholders by ROLE_PRIORITY produces CTO > CIO > CFO > CMO > others."""
        from production_main import ROLE_PRIORITY

        stakeholders = [
            {"role_type": "CISO", "name": "S"},
            {"role_type": "CMO", "name": "D"},
