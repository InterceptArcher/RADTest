...
class GammaSlideshowCreator:
    ...
    def _generate_markdown(self, company_data: Dict[str, Any], user_email: str = None) -> str:
        ...
        if not intent_topics:
            industry = validated_data.get('industry', 'technology')
            # Generate industry-relevant topics dynamically
            base_topics = []
            if any(word in industry.lower() for word in ['manufacturing', 'automotive', 'industrial']):
                base_topics.append({'topic': 'Digital Manufacturing & IoT', 'score': 85, 'what_this_means': f'Active focus on industrial modernization suggests opportunity for edge computing, connected device infrastructure, and operational technology solutions in the {industry} sector.'})
            if any(word in industry.lower() for word in ['financial', 'banking', 'insurance']):
                base_topics.append({'topic': 'Financial Services Security & Compliance', 'score': 88, 'what_this_means': f'Elevated regulatory focus in {industry} indicates need for secure infrastructure, compliance-ready systems, and risk management solutions.'})
            if any(word in industry.lower() for word in ['healthcare', 'medical', 'hospital']):
                base_topics.append({'topic': 'Healthcare IT Modernization', 'score': 82, 'what_this_means': f'Healthcare digital transformation suggests opportunity for secure patient data infrastructure and compliance-ready technology in {industry}.'})

            # Add universal topics if industry-specific aren't enough
            if len(base_topics) < 3:
                base_topics.append({'topic': 'Cloud & Infrastructure Modernization', 'score': 80, 'what_this_means': f'{company_name} appears to be evaluating cloud solutions, suggesting opportunity for hybrid infrastructure and managed services partnerships.'})
            if len(base_topics) < 3:
                base_topics.append({'topic': 'Cybersecurity & Risk Management', 'score': 75, 'what_this_means': f'Security focus indicates need for endpoint protection, secure infrastructure, and compliance-ready solutions in {industry}.'})
            if len(base_topics) < 3:
                base_topics.append({'topic': 'Digital Workplace Transformation', 'score': 72, 'what_this_means': f'Workplace modernization initiatives suggest opportunity for device fleet management, collaboration tools, and productivity solutions.'})

            intent_topics = base_topics[:3]
        ...
        for i, topic in enumerate(intent_topics[:3], 1):
            if isinstance(topic, dict):
                ...
                what_this_means = topic.get('what_this_means', f'This signal indicates {company_name} is actively researching solutions in this area, presenting an opportunity for HP to engage with relevant offerings.')
            else:
                topic_name = str(topic)
                topic_score = 80 - i*5
                what_this_means = f'This signal indicates {company_name} is actively researching solutions in this area.'

            markdown += f"**[{i}] {topic_name}**\n\n"
            markdown += f"Score: {topic_score}\n\n"
            markdown += f"**What this means:** {what_this_means}\n\n"

        # Interest over time chart
        markdown += "## Interest Over Time\n\n"
        ...
        markdown += "\n"
        markdown += f"*{company_name} is showing intent-signal activity in these areas. Use these scores as the \"why now\" opener, then qualify quickly: what's driving the activity, which sites/users are in scope, timing, and the decision path.*\n\n"
        ...
        markdown += "## Feedback form\n\n"
        markdown += "Let us know what you think. Your feedback helps us make future reports even more relevant and useful.\n\n"
        markdown += "<Share your thoughts>\n\n"

        markdown += "## Questions\n\n"
        markdown += "If you have any questions about this report, contact the HP RAD Intelligence Desk at [email@hp.com]\n\n"
