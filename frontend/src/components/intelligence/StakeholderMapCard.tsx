      ...
      <div className={`${roleConfig.bgColor} px-4 py-3 border-b border-slate-200`}>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            {stakeholder.factCheckScore != null && stakeholder.factCheckScore > 0 && (
              <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${
                stakeholder.factCheckScore >= 0.8 ? 'bg-emerald-100 text-emerald-700' :
                stakeholder.factCheckScore >= 0.5 ? 'bg-amber-100 text-amber-700' :
                'bg-red-100 text-red-700'
              }`}>
                {stakeholder.factCheckScore >= 0.8 ? 'Verified' : stakeholder.factCheckScore >= 0.5 ? 'Unverified' : 'Low Confidence'}
              </span>
            )}
...
export default function StakeholderMapCard({ stakeholderMap, supportingAssets, onGenerateOutreach }: StakeholderMapCardProps) {
  ...
  // Preview: show from executives first, then fill from other contacts
  const allContacts = [...stakeholders, ...otherContacts];
  const previewStakeholders = allContacts.slice(0, 3);
    ...
    <div className="card overflow-hidden">
      ...
      {expanded && (
        <div className="px-6 pb-6 border-t border-slate-100">
          ...
          {otherCount > 0 && (
            <div className={executiveCount > 0 ? "mt-6 border-t border-slate-200 pt-6" : "mt-6"}>
              <div className="flex items-center justify-between mb-4">
                ...
                <p className="text-xs text-slate-500">
                  {executiveCount > 0 ? 'VPs, Directors, and key personnel' : 'Decision makers and influencers'}
                </p>
              </div>
              <div className="space-y-4">
                {otherContacts.map((contact, index) => (
                  executiveCount === 0 ? (
                    <StakeholderDetailCard
                      ...
                      stakeholder={contact}
                      supportingAsset={findSupportingAsset(contact)}
                      onGenerateOutreach={onGenerateOutreach}
                    />
                  ) : (
          ...
          {/* No contacts at all */}
          {totalCount === 0 && (
            <div className="mt-6 bg-slate-50 rounded-xl p-8 text-center">
