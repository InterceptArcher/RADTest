...
**Floor-fill prefers the web agent over a weak ZI contact.** Evaluation across Microsoft / Sun Life / BC Liquor showed that on large domains ZoomInfo returns a generic company-wide pool with *no* title match for a given persona; the relevance-first selection then leaves that persona empty and floor-fill used to keep a complete-but-wrong ZI contact (the "most data" trap). Floor-fill now keeps a catalogue contact ONLY if it's a real-ish match (≤ Director); otherwise it calls the web-search fallback agent for the *actual* role-holder (proven to find Kevin Scott / Amy Hood / Carolina Dybeck Happe on the Microsoft Canada run). The right person with a LinkedIn URL beats the wrong person with a full contact card. (Known follow-up: web-found execs currently carry LinkedIn + best-effort start date but not email/phone, because ZoomInfo Contact-Enrich matches by `personId` only — a name-based enrich path would fill those and is the proposed next step.)

**Tests:** new `test_targeted_job_titles_searched_before_generic_clevel`, `test_canada_only_restricts_country_and_skips_global`, `test_floor_fill_prefers_web_agent_over_weak_catalogue_contact`, `deck_basename` Canada-suffix test; existing selection/IO tests updated for the relevance-first stop (co-equal-best). `start_date` stays best-effort (citation-gated web search) — it is intentionally NOT a selection or enrichment hard requirement (phone/email/LinkedIn are).
